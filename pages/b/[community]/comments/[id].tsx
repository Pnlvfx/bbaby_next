...
} from '../../../../components/community/CommunityContext'
...
const IdPage: NextPage<PostIdPageProps> = ({ post, redirect }) => {
  ...
  return (
    <>
      <CEO
        title={post?.title}
        url={`${siteUrl}/b/${post.community.toLowerCase()}/comments/${
          post._id
        }`}
        description={
        ...
        width={
          post.mediaInfo?.dimension
            ? post.mediaInfo.dimension[1]?.toString()
            : undefined
        }
        height={
          post.mediaInfo?.dimension
            ? post.mediaInfo.dimension[0]?.toString()
            : undefined
        }
        index={true}
      />
