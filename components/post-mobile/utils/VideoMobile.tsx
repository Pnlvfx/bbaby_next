...
const VideoMobile = ({ post }: PostMobile) => {
  return (
    <div
      ...
      style={{ '--max-height': '100%', '--aspect-ratio': `${post.mediaInfo?.dimension[1]} / ${post.mediaInfo?.dimension[0]}` } as CSSProperties}
    >
      <div
        className="videoMobile"
        style={{ '--max-height': '100%', '--aspect-ratio': `${post.mediaInfo?.dimension[1]} / ${post.mediaInfo?.dimension[0]}` } as CSSProperties}
      >
        <div
          ...
          style={{ '--max-height': '100%', '--aspect-ratio': `${post.mediaInfo?.dimension[1]} / ${post.mediaInfo?.dimension[0]}` } as CSSProperties}
        >
          <link className="hidden" rel="preload" href={post?.mediaInfo?.video.url.replace('.mp4', '.jpg')} as="image" />
          <div
            className="videoMobile block h-full max-h-[100%] w-full"
            style={{ '--max-height': '100%', '--aspect-ratio': `${post.mediaInfo?.dimension[1]} / ${post.mediaInfo?.dimension[0]}` } as CSSProperties}
          >
            <div
              style={
                { '--max-height': '100%', '--aspect-ratio': `${post.mediaInfo?.dimension[1]} / ${post.mediaInfo?.dimension[0]}` } as CSSProperties
              }
            >
              <div className="inline-block h-full w-full">
                <div className="inline-block h-full w-full">
                  <video
                    ...
                    className="video videoSlot absolute"
                  >
